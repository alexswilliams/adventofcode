...
private data class Hand(val cardOrderStrength: Int, val type: PokerType, val bid: Int)
...
private fun part1(input: List<String>): Int {
    return input.map { it.split(' ') }
        .map { (cards, bid) -> Hand(cardOrderingStrength(cards, part1Ordering), classifyCards(cards), bid.toInt()) }
...
private fun part2(input: List<String>): Int {
    return input.map { it.split(' ') }
        .map { (cards, bid) -> Hand(cardOrderingStrength(cards, part2Ordering), classifyCardsWithJoker(cards), bid.toInt()) }
        .sortedWith(compareBy<Hand> { hand -> hand.type.strength }.thenComparing { hand -> hand.cardOrderStrength })
        .withIndex()
        .sumOf { (index, hand) -> (index + 1) * hand.bid }
}
...
private fun classifyCardsWithJoker(cards: String): PokerType {
    ...
    if (jokerCount >= 4) return PokerType.FIVE_O_A_K
...
private fun classifyCards(cards: String): PokerType {
    val groups = cards.frequency()
    return when {
        groups.size == 1 -> PokerType.FIVE_O_A_K
        groups.size == 2 && groups.any { it.second == 4 } -> PokerType.FOUR_O_A_K
        groups.size == 2 && groups.any { it.second == 3 } -> PokerType.FULL_HOUSE
        groups.size == 3 && groups.any { it.second == 3 } -> PokerType.THREE_O_A_K
        groups.size == 3 && groups.any { it.second == 1 } && groups.any { it.second == 2 } -> PokerType.TWO_PAIR
        groups.size == 4 -> PokerType.ONE_PAIR
